...
class SiPMAna:
    ...
    def PlotEvent(self):
        ...
        ax[0, 0].plot(x, np.array(self.ds_low[1]), color='blue', label='Raw Waveform')
        ax[0, 0].plot(x, filtered_wf1, color='red', label='Filtered Waveform')
        ax[0, 0].set_xlabel('Time (ns)')
        ax[0, 0].set_ylabel('Magnitude (V)')
        ax[0, 0].set_title('Channel 1 30 V Bias')
        ax[0, 0].set_ylim(0.055, 0.1)
        ax[0, 0].legend()
        ...
        #ax[1, 1].legend()
        ax[2, 0].hist(np.array(self.ds_low[1]), bins=np.linspace(0.055, 0.070, 16), histtype='step', color='blue', label='Raw')
        ax[2, 0].hist(filtered_wf1, bins=np.linspace(0.055, 0.070, 16), histtype='step', color='red', label='Filtered')
        ax[2, 0].text(0.056, 500000, r'$\sigma$=%.4f V' % np.std(filtered_wf1))
        ax[2, 0].set_xlabel('Baseline (V)')
        ax[2, 0].set_title('Channel 1 Baseline')
        #ax[2, 0].legend()
        ax[2, 1].hist(np.array(self.ds_low[2]), bins=np.linspace(0.055, 0.070, 16), histtype='step', color='blue', label='Raw')
        ax[2, 1].hist(filtered_wf2, bins=np.linspace(0.055, 0.070, 16), histtype='step', color='red', label='Filtered')
        ax[2, 1].text(0.056, 500000, r'$\sigma$=%.4f V' % np.std(filtered_wf2))
